...
@implementation SPBrowserImageView

{
    __weak SPWaitingView *_waitingView;
    BOOL _didCheckSize;
...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = SPPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
{
    SPWaitingView *waiting = [[SPWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = SPWaitingViewProgressMode;
    _waitingView = waiting;
    [self addSubview:waiting];
    
    
    __weak SPBrowserImageView *imageViewWeak = self;
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = SPPhotoBrowserBackgrounColor;
